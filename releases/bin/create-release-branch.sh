...
${command_name} [--major MAJOR_VERSION] [--minor MINOR_VERSION] [--commit GIT_COMMIT] [--recreate] [--dry-run] [--help | -h]
  ...
  When invoked from a major-version branch, a new minor-version branch '${release_branch_prefix}/<major>.<minor>.x' will be created.
...
case "${branch_type}" in
  ...
  "major")
    ...
    if [[ $max_minor -eq -1 ]]; then
      ...
      new_branch_name="${release_branch_prefix}${version_major}.0.x"
    else
      echo "Latest major release branch is for version ${max_major}.x"
      new_branch_name="${release_branch_prefix}${version_major}.$(( max_minor + 1)).x"
    fi
    ;;
  "minor")
