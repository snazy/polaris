...
function usage {
  cat <<!
Usage: $0 --major <major-version> --minor <minor-version> --patch <patch-version> --label <version-labels> --previous [<prev-version-full>]
  ...
  version-labels          FULL version labels string, including leading '-', including leading '-incubating'
                          Examples:
                              -incubating
                              -incubating-beta
...
prev_tag="$(tag_for_full_version "${prev_version_full}")"
prev_version_full="$(full_version_with_label_from_release_tag "${prev_tag}")"
...
In particular for this release, a big thank you to

$(git rev-list --reverse HEAD ^"${prev_tag}" --format="%an" | grep --invert-match --extended-regexp "commit |dependabot\[bot\]|renovate\[bot\]" | sort | uniq | paste -s -d, | sed 's/,/, /g').

$([[ -f "${worktree_dir}/NEXT_RELEASE_NOTES.md" ]] && tail -n+23 "${worktree_dir}/NEXT_RELEASE_NOTES.md")
...
git rev-list --reverse HEAD ^"${prev_tag}" --format="${sep_start}%+H,%h,%an%+s%+b%n${sep_end}" | while read -r ln ; do
  case $in_log in
    0)
