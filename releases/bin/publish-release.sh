...
${command_name}
  ...
  This command looks up the latest release candidate for the latest patch version of the given major/minor
  version and promotes it. Promotion will fail, if a promoted tag (no '-rcX' suffix) for the latest patch
...
if [[ $patch_version -eq -1 ]]; then
  ...
  from_tag_name="$(get_latest_rc_tag_name "${version_full_base}")"
  if [[ -z ${from_tag_name} ]]; then
    # that patch version is released
    echo "Version ${version_full} is already released, aborting" > /dev/stderr
    exit 1
  fi
fi
version_label="$(version_label_from_rc_tag "${from_tag_name}")"
version_full="${version_full_base}${version_label}"
# shellcheck disable=SC2154
...
git worktree add "${worktree_dir}" "${from_tag_name}"
cd "${worktree_dir}"
echo "Changed to directory $(pwd)"
