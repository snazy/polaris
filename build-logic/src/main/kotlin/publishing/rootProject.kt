...
/**
 * Configures Apache project specific publishing tasks on the root project, for example the source-tarball related tasks.
 */
internal fun configureOnRootProject(project: Project) = project.run {
    ...
    sourceTarball.configure(){
        group = "build"
        description = "Generate a source tarball for a release to be uploaded to dist.apache.org/repos/dist"
        ...
        doFirst {
            ...
            exec {
                executable = "git"
                args("archive", "--prefix=${e.baseName.get()}/", "--format=tar.gz", "--output=${e.sourceTarball.get().asFile.relativeTo(projectDir)}", "HEAD")
    ...
    sourceTarball.configure {
        finalizedBy(digestSourceTarball)
    }
