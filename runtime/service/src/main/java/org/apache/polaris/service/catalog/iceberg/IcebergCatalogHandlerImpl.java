...
public abstract class IcebergCatalogHandlerImpl extends CatalogHandlerBase
    implements IcebergCatalogHandler {
  private static final Logger LOGGER = LoggerFactory.getLogger(IcebergCatalogHandlerImpl.class);
  ...
  public static Builder builder() {
    return ImmutableIcebergCatalogHandlerImpl.builder();
  }

  public interface Builder {
    @CanIgnoreReturnValue
    Builder from(CatalogHandler instance);

    @CanIgnoreReturnValue
    Builder catalogName(String catalogName);

    @CanIgnoreReturnValue
    Builder polarisPrincipal(PolarisPrincipal polarisPrincipal);

    @CanIgnoreReturnValue
    Builder callContext(CallContext callContext);

    @CanIgnoreReturnValue
    Builder metaStoreManager(PolarisMetaStoreManager metaStoreManager);

    @CanIgnoreReturnValue
    Builder resolutionManifestFactory(ResolutionManifestFactory resolutionManifestFactory);

    @CanIgnoreReturnValue
    Builder authorizer(PolarisAuthorizer authorizer);

    @CanIgnoreReturnValue
    Builder diagnostics(PolarisDiagnostics diagnostics);

    @CanIgnoreReturnValue
    Builder credentialManager(PolarisCredentialManager credentialManager);

    @CanIgnoreReturnValue
    Builder externalCatalogFactories(Instance<ExternalCatalogFactory> externalCatalogFactories);

    @CanIgnoreReturnValue
    Builder prefixParser(CatalogPrefixParser prefixParser);

    @CanIgnoreReturnValue
    Builder resolverFactory(ResolverFactory resolverFactory);

    @CanIgnoreReturnValue
    Builder catalogFactory(CallContextCatalogFactory catalogFactory);

    @CanIgnoreReturnValue
    Builder reservedProperties(ReservedProperties reservedProperties);

    @CanIgnoreReturnValue
    Builder catalogHandlerUtils(CatalogHandlerUtils catalogHandlerUtils);

    @CanIgnoreReturnValue
    Builder storageAccessConfigProvider(StorageAccessConfigProvider storageAccessConfigProvider);

    @CanIgnoreReturnValue
    Builder eventAttributeMap(EventAttributeMap eventAttributeMap);

    @CanIgnoreReturnValue
    Builder metricsReporter(PolarisMetricsReporter metricsReporter);

    @CanIgnoreReturnValue
    Builder clock(Clock clock);

    IcebergCatalogHandlerImpl build();
  }
}
